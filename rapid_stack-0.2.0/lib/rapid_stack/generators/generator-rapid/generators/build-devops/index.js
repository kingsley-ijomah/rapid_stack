...
module.exports = class extends Generator {
  ...
  async _setupAnsible() {
    const { confirmAnsible } = await this.prompt([{
      type: 'confirm',
      name: 'confirmAnsible',
    ...
    if (confirmAnsible) {
      ...
      this.log('✓ Ansible playbook created');
    }
  }

  async _setupGraylog() {
    const { confirmGraylog } = await this.prompt([{
      type: 'confirm',
      name: 'confirmGraylog',
  ...
  async _setupDockerComposeFiles() {
    const { confirmDockerComposeFiles } = await this.prompt([{
      type: 'confirm',
  ...
  async _setupDeployScript() {
    const { confirmDeployScript } = await this.prompt([{
      type: 'confirm',
  ...
  async _setupTerraform() {
    const { confirmTerraform } = await this.prompt([{
      type: 'confirm',
      name: 'confirmTerraform',
      message: 'Would you like to set up the Terraform configuration?',
      default: true
    }]);
