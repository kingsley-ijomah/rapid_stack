...
describe('ShellManager', () => {
  ...
  describe('addASDFPaths', () => {
    const configFile = '/mock/home/dir/.zshrc';
    const asdfPaths = [
      '/mock/home/dir/.asdf/shims',
      '/mock/home/dir/.asdf/bin'
    ];
    ...
    it('should add PATH line at the beginning when file is empty', async () => {
      const result = await shellManager.addASDFPaths();
      expect(result).toBe(true);
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        configFile,
        expect.stringContaining(`export PATH="${asdfPaths.join(':')}:$PATH"`)
      );
    });
    ...
    it('should update existing non-appending PATH line', async () => {
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        configFile,
        expect.stringContaining(`export PATH="${asdfPaths.join(':')}:/usr/local/bin:$PATH"`)
      );
    });
    ...
    it('should not duplicate ASDF paths when running multiple times', async () => {
      // First run
      fs.readFileSync.mockReturnValue('export PATH="/usr/local/bin:$PATH"\n');
      await shellManager.addASDFPaths();
      ...
      // Check that ASDF paths appear only once
      const finalContent = fs.writeFileSync.mock.calls[1][1];
      const asdfPathCount = (finalContent.match(/\.asdf\/shims/g) || []).length;
      expect(asdfPathCount).toBe(1);
    });
