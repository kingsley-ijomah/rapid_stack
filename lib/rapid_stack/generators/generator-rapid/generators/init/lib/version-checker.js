...
class VersionChecker {
  ...
  displayVersionComparison() {
    ...
    const versions = {};
    const totalTools = Object.keys(this.minVersions).length;
    let currentTool = 0;

    // Check each tool version with progress
    for (const [tool, minVersion] of Object.entries(this.minVersions)) {
      currentTool++;
      this.showProgress(currentTool, totalTools, tool);
      
      versions[tool] = this.getCommandVersion(tool === 'angular' ? 'ng' : tool);
    }
