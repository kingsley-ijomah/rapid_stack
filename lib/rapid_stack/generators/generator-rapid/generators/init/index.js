        ...
        try {
          ...
          if (issues.missing.includes(tool)) {
            ...
            if (versions.success && versions.versions.length > 0) {
              this.log(`\n📦 Available versions for ${packageInfo.name}:`);
              ...
              const { action } = await this.prompt([
                {
                  type: 'list',
                  name: 'action',
                  message: `How would you like to install ${packageInfo.name}?`,
                  choices: [
              ...
              if (action === 'skip') {
                this.log(`⏭️ Skipping installation of ${packageInfo.name}`);
                continue;
              }

              // Show installation progress
              this.log(`\n🚀 Starting installation of ${packageInfo.name}...`);
              try {
                ...
                if (result) {
                  this.log(`✅ Successfully installed ${packageInfo.name}`);
                  ...
                  await this.shellManager.reloadConfig();
                  this.log(`✅ Path configuration complete for ${packageInfo.name}`);
                } else {
                  this.log(`❌ Failed to install ${packageInfo.name}`);
                }
              } catch (error) {
                this.log(`❌ Error during installation of ${packageInfo.name}: ${error.message}`);
              }
            } else {
              this.log(`❌ No versions found for ${packageInfo.name}`);
            }
          } else {
