...
module.exports = class extends Generator {
  ...
  async prompting() {
    ...
    this.answers = {
      projectName: 'backend',
      railsVersion: '8.0.1'  // Set a default Rails version
    };
  ...
  async checkPrerequisites() {
    try {
      ...
      try {
        ...
        const { installRails } = await this.prompt([{
          type: 'confirm',
          name: 'installRails',
      ...
      try {
        ...
        const { continueMongo } = await this.prompt([{
          type: 'confirm',
          name: 'continueMongo',
  ...
  async _setupRailsProject() {
    ...
    if (!fs.existsSync(projectPath)) {
      const { confirmCreate } = await this.prompt([{
        type: 'confirm',
        name: 'confirmCreate',
  ...
  async _setupGemfile() {
    ...
    try {
      const { confirmGemfile } = await this.prompt([{
        type: 'confirm',
        name: 'confirmGemfile',
  ...
  async _setupMongoid() {
    const { confirmMongoid } = await this.prompt([{
      type: 'confirm',
      name: 'confirmMongoid',
  ...
  async _setupGraphQL() {
    const { confirmGraphQL } = await this.prompt([{
      type: 'confirm',
      name: 'confirmGraphQL',
  ...
  async _setupRoutes() {
    const { confirmRoutes } = await this.prompt([{
      type: 'confirm',
      name: 'confirmRoutes',
  ...
  async _setupCORS() {
    const { confirmCORS } = await this.prompt([{
      type: 'confirm',
      name: 'confirmCORS',
  ...
  async _setupJWTHelper() {
    const { confirmJWTHelper } = await this.prompt([{
      type: 'confirm',
  ...
  async _setupRSpec() {
    const { confirmRSpec } = await this.prompt([{
      type: 'confirm',
      name: 'confirmRSpec',
  ...
  async _setupGraphQLHelper() {
    const { confirmGraphQLHelper } = await this.prompt([{
      type: 'confirm',
  ...
  async _setupMailers() {
    const { confirmMailers } = await this.prompt([{
      type: 'confirm',
      name: 'confirmMailers',
  ...
  async _setupGuard() {
    const { confirmGuard } = await this.prompt([{
      type: 'confirm',
      name: 'confirmGuard',
  ...
  async _setupServices() {
    const { confirmServices } = await this.prompt([{
      type: 'confirm',
  ...
  async _generateGraphQL() {
    const { confirmGenerateGraphQL } = await this.prompt([{
      type: 'confirm',
  ...
  async _removeMasterKey() {
    ...
    const { confirmRemoveMasterKey } = await this.prompt([{
      type: 'confirm',
  ...
  async _setUpEnv() {
    const { confirmEnv } = await this.prompt([{
      type: 'confirm',
      name: 'confirmEnv',
  ...
  async _setUpConfigHelper() {
    const { confirmConfigHelper } = await this.prompt([{
      type: 'confirm',
  ...
  async _updateApplicationConfig() {
    const { confirmUpdateApplicationConfig } = await this.prompt([{
      type: 'confirm',
  ...
  async _removeActiveRecordAppConfig() {
    const { confirmRemoveActiveRecordAppConfig } = await this.prompt([{
      type: 'confirm',
  ...
  async _setupDockerfile() {
    const { confirmDockerfile } = await this.prompt([{
      type: 'confirm',
  ...
  async _setupGithubActions() {
    const { confirmGithubActions } = await this.prompt([{
      type: 'confirm',
  ...
  async _bundleInstall() {
    ...
    // Install dependencies
    const { runBundleInstall } = await this.prompt([{
      type: 'confirm',
  ...
  async _copyTemplateFile(templatePath, destinationPath, context = {}, options = { force: true }) {
    try {
      ...
      if (fs.existsSync(destinationFullPath) && !options.force) {
        const { shouldOverwrite } = await this.prompt([{
          type: 'confirm',
