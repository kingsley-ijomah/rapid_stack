...
class AuthGenerator extends Generator {
  ...
  async initializing() {
    ...
    try {
      const jwtDenylistTemplate = fs.readFileSync(this.templatePath('jwt_denylist.rb.erb'), 'utf8');
    ...
    try {
      const otpTemplate = fs.readFileSync(this.templatePath('otp.rb.erb'), 'utf8');
  ...
  _generateMutationFiles(destPath) {
    ...
    mutations.forEach(mutation => {
      if (mutation.include) {
        ...
        this.fs.copyTpl(
          this.templatePath(`${mutation.name}.rb.ejs`),
