...
module.exports = class extends Generator {
  ...
  async initializing() {
    ...
    const requiredFolders = [
      { name: 'frontend', command: 'frontend' },
      { name: 'backend', command: 'backend' },
      { name: 'devops', command: 'devops' },
      { name: 'nginx', command: 'nginx' }
    ];
    ...
    if (missingFolders.length > 0) {
      ...
      missingFolders.forEach(folder => {
        console.log(`rapid build:${folder.command}`);
      });
      
      console.log('\nAfter building all components, run this command again:');
      console.log('rapid build:terraform');
