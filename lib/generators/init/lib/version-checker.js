...
class VersionChecker {
  ...
  getCommandVersion(command) {
    try {
      ...
      // Find the package info for this command
      const packageInfo = Object.values(packages).find(pkg => 
        pkg.versionCommand.split(' ')[0] === command
      );
      ...
      // Use the versionCommand from package info
      const version = execSync(packageInfo.versionCommand).toString().trim();
  ...
  displayVersionComparison() {
    ...
    for (const [tool, packageInfo] of Object.entries(packages)) {
      ...
      // Use versionCommand to check version
      const command = packageInfo.versionCommand.split(' ')[0];
      versions[tool] = this.getCommandVersion(command);
    }
