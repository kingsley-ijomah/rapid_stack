...
module.exports = class extends Generator {
  ...
  async prompting() {
    ...
    // Create .rapidrc file
    const rapidrcPath = path.join(fullPath, '.rapidrc');
    const rapidrcContent = JSON.stringify({ projectName: path.basename(fullPath) }, null, 2);
    fs.writeFileSync(rapidrcPath, rapidrcContent);
    this.log(`Created .rapidrc file: ${rapidrcPath}`);
  ...
  async writing() {
    ...
    if (this.answers.usingExistingConfig) {
      ...
      console.log('\n2. Build fullstack application:');
      console.log('   rapid build:fullstack');
    ...
    // Create .rapidrc file
    const rapidrcPath = path.join(this.answers.fullPath, '.rapidrc');
    const rapidrcContent = JSON.stringify({ projectName: path.basename(this.answers.fullPath) }, null, 2);
    fs.writeFileSync(rapidrcPath, rapidrcContent);
    this.log(`Created .rapidrc file: ${rapidrcPath}`);
