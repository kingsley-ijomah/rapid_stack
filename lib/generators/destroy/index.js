...
const { validateProjectConfig, findProjectRoot } = require('../../lib/utils');
...
module.exports = class extends BaseGenerator {
  ...
  async initializing() {
    ...
    this.configPath = path.join(rapidStackDir, `${this.appName}_project.yml`);
    if (!fs.existsSync(this.configPath)) {
      this.log('\n' + '='.repeat(80));
      this.log('❌ Project Configuration Not Found!');
      this.log('='.repeat(80));
      this.log('\n📝 Expected file:');
    ...
    this.projectConfig = validateProjectConfig(this.appName);
  }
  ...
  async _deleteGitHubRepos() {
    ...
    const token = this.projectConfig.config.repo_access_token;
    if (!token) {
