...
module.exports = class extends BaseGenerator {
  ...
  async _setupGitHubRepository() {
    try {
      ...
      const cloudflareApiKey = getConfigField('cloudflare_api_key');
      const cloudflareAccountId = getConfigField('cloudflare_account_id');
      ...
      if (createRepo) {
        // Validate Cloudflare fields
        const missingFields = [];
        if (!cloudflareApiKey) missingFields.push('cloudflare_api_key');
        if (!cloudflareAccountId) missingFields.push('cloudflare_account_id');
        ...
        try {
          const repoName = `${appName}-frontend`;
          await createGitHubRepos(
            getConfigField('github_username'),
            getConfigField('repo_access_token'),
            repoName,
          ...
          await initializeAndPushGitRepo(getConfigField('github_username'), repoName);
