...
module.exports = class extends Generator {
  ...
  async install() {
    ...
    try {
      if (!projectExists) {
        ...
        execSync(`ionic start ${projectName} blank --type=angular --capacitor`, { stdio: 'inherit' });
  ...
  async _setupGitHubActions(projectPath) {
    const workflowPath = path.join(projectPath, '.github', 'workflows', 'frontend-build.yml');
    ...
    this.fs.copy(
      this.templatePath('frontend-build.yml.erb'),
      workflowPath
    );

    this.log('✓ Added GitHub Actions workflow for frontend build and deployment');
  }
  ...
  async _updateAngularJson(projectPath) {
    ...
    try {
      ...
      if (angularJson.projects && angularJson.projects.app && angularJson.projects.app.architect) {
        ...
        if (architect.build) {
          architect.build.options.outputPath = "www/frontend";
  ...
  async _setupWranglerConfig(projectPath) {
    ...
    const interpolatedContent = templateContent.replace(/<%= compatibility_date %>/g, currentDate);
