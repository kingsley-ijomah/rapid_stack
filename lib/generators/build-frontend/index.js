...
module.exports = class extends Generator {
  ...
  async prompting() {
    ...
    const configPath = path.join(process.env.HOME, '.rapid_stack', `${parentDir}_project.yml`);
    
    let appName = 'frontend';
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        // Check if app_name exists in config
        if (config?.config?.app_name) {
          appName = config.config.app_name;
        }
      } catch (error) {
        this.log('Error reading config file:', error.message);
      }
    }
