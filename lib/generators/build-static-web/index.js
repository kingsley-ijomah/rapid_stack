    ...
    try {
      ...
      try {
        ...
        try {
          ...
          this.log('Installing npx...');
          execSync('npm install -g npx', { stdio: 'inherit' });
        }
        
        // Now run supabase init
        execSync('npx supabase init', { stdio: 'inherit' });
        this.log('✓ Supabase initialized successfully');
      } catch (error) {
        this.log.error('Error initializing Supabase:', error.message);
        this.log('You can manually initialize Supabase later with: npm install -g npx && npx supabase init');
      }
