...
module.exports = class extends BaseGenerator {
  ...
  async _setupGitIgnore() {
    this._copyTemplateFile('.gitignore.erb', '.gitignore', {});
    this.log(`✓ Created .gitignore: ${this._fileLink('.gitignore')}`);
  }
  ...
  async _setupComposefiles() {
    const { confirmComposefiles } = await handlePrompt(this, [{
      type: 'confirm',
      name: 'confirmComposefiles',
      message: 'Would you like to set up the compose files?',
      default: true
    }]);

    if (confirmComposefiles) {
      this._copyTemplateFile('docker-compose.prod.yml.erb', 'docker-compose.prod.yml', {});
      this.log(`✓ Compose files created: ${this._fileLink('docker-compose.yml')}`);
    }
  }
  ...
  async _setupEnvFile() {
    try {
      // Read the template file
      const envTemplate = fs.readFileSync(this.templatePath('env.erb'), 'utf8');

      // Generate secure random values
      const jwtSecretKey = execSync('openssl rand -hex 64').toString().trim();
      const secretKeyBase = execSync('openssl rand -hex 64').toString().trim();
      const railsMasterKey = execSync('openssl rand -hex 16').toString().trim();

      // Replace the placeholders with generated values
      const envContent = envTemplate
        .replace('JWT_SECRET_KEY=', `JWT_SECRET_KEY=${jwtSecretKey}`)
        .replace('SECRET_KEY_BASE=', `SECRET_KEY_BASE=${secretKeyBase}`)
        .replace('RAILS_MASTER_KEY=', `RAILS_MASTER_KEY=${railsMasterKey}`);

      // Write the .env file
      const envPath = path.join(this.destinationPath(), '.env');
      fs.writeFileSync(envPath, envContent);

      this.log(`✓ Created .env file with secure keys: ${this._fileLink('.env')}`);
    } catch (error) {
      this.log('Error setting up .env file:', error.message);
      if (this.options.debug) {
        this.log('Stack trace:', error.stack);
  ...
  // Helper method to generate secure key using openssl
  ...
  // Helper method to create GitHub secrets
  async _createGitHubSecrets(githubUsername, repoName, repoAccessToken, appName, config) {
    try {
      ...
      // Get SSH key content
      const sshFiles = fs.readdirSync(path.join(process.env.HOME, '.ssh'));
      const matchingKey = sshFiles.find(file => file.startsWith('rapid_stack_'));
      if (!matchingKey) {
        this.log('❌ No SSH key found starting with "rapid_stack_" in ~/.ssh directory');
        process.exit(1);
      }
      const sshKeyContent = fs.readFileSync(path.join(process.env.HOME, '.ssh', matchingKey), 'utf8');

      // Set up GitHub secrets
      this.log('Setting up GitHub secrets...');
      const secrets = [
        { key: 'MY_GITHUB_USERNAME', value: githubUsername },
        { key: 'APP_NAME', value: appName },
        { key: 'REPO_ACCESS_TOKEN', value: repoAccessToken },
        { key: 'DOCKERHUB_USERNAME', value: config.dockerhubUsername },
        { key: 'DOCKERHUB_PASSWORD', value: config.dockerhubPassword },
        { key: 'REMOTE_MACHINE', value: config.remoteMachine },
        ...
        { key: 'APP_SUPPORT_EMAIL', value: config.appSupportEmail },
        { key: 'MAILER_FROM_NAME', value: config.mailerFromName },
        { key: 'MAILER_FROM_ADDRESS', value: config.mailerFromAddress },
      ...
      this.log('✓ GitHub secrets set successfully');
      this.log('✓ MongoDB secrets generated:');
      this.log(`  - Database: ${mongodbDatabase}`);
      this.log(`  - User: ${mongodbUser}`);
      this.log(`  - Password: ${mongodbPassword}`);
      this.log('✓ Secure keys generated:');
      this.log(`  - JWT Secret Key: ${jwtSecretKey}`);
      this.log(`  - Secret Key Base: ${secretKeyBase}`);
      this.log(`  - Rails Master Key: ${railsMasterKey}`);
      this.log('✓ Found SSH key:', matchingKey);
    } catch (error) {
  ...
  async _setupGitHubRepository() {
    try {
      ...
      const githubUsername = getConfigField('github_username');
      const appName = getConfigField('app_name');
      const repoAccessToken = getConfigField('repo_access_token');
      const dockerhubUsername = getConfigField('dockerhub_username');
      const dockerhubPassword = getConfigField('dockerhub_password');
      const remoteMachine = getConfigField('remote_machine');
      const appSupportEmail = getConfigField('app_support_email');
      const mailerFromName = getConfigField('mailer_from_name');
      const mailerFromAddress = getConfigField('mailer_from_address');
      const postmarkApiKey = getConfigField('postmark_api_key');
      
      // Ask user if they want to create the repository
      const { createRepo } = await handlePrompt(this, [{
        type: 'confirm',
        name: 'createRepo',
        message: `Would you like to create the ${appName}-backend repository?`,
        default: true
      }]);
      ...
      if (createRepo) {
        ...
        const missingFields = [];
        if (!githubUsername) missingFields.push('github_username');
        if (!appName) missingFields.push('app_name');
        if (!repoAccessToken) missingFields.push('repo_access_token');
        if (!dockerhubUsername) missingFields.push('dockerhub_username');
        if (!dockerhubPassword) missingFields.push('dockerhub_password');
        if (!remoteMachine) missingFields.push('remote_machine');
        if (!appSupportEmail) missingFields.push('app_support_email');
        if (!mailerFromName) missingFields.push('mailer_from_name');
        if (!mailerFromAddress) missingFields.push('mailer_from_address');
        if (!postmarkApiKey) missingFields.push('postmark_api_key');
        if (missingFields.length > 0) {
          this.log('❌ Required fields missing in config:');
          missingFields.forEach(field => this.log(`  - ${field}`));
          this.log('Please update your config file and try again.');
          process.exit(1);
        }
        ...
        try {
          const repoName = `${appName}-backend`;
          ...
          this.log('Setting up GitHub secrets before initial push...');
          await this._createGitHubSecrets(githubUsername, repoName, repoAccessToken, appName, {
            dockerhubUsername,
            dockerhubPassword,
            remoteMachine,
            appSupportEmail,
  ...
  async _bundleInstall() {
    ...
    if (runBundleInstall) {
      ...
      // Remove existing devise.rb if it exists
      const devisePath = this.destinationPath('config/initializers/devise.rb');
      if (fs.existsSync(devisePath)) {
        fs.unlinkSync(devisePath);
      }
