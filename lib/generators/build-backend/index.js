...
module.exports = class extends BaseGenerator {
  ...
  async _setupGitHubRepository() {
    try {
      ...
      // Look for SSH key
      const sshKeyPath = path.join(process.env.HOME, '.ssh', 'rapid_stack_');
      ...
      try {
        ...
        if (matchingKey) {
          sshKeyContent = fs.readFileSync(path.join(process.env.HOME, '.ssh', matchingKey), 'utf8');
        ...
        this.log('❌ Error reading SSH key:', error.message);
        process.exit(1);
      }
