...
module.exports = class extends Generator {
  ...
  async _setupAnsible() {
    ...
    if (confirmAnsible) {
      ...
      // Copy Ansible roles
      const rolesSourcePath = this.templatePath('ansible/roles');
      const rolesDestPath = this.destinationPath('ansible/roles');

      // Create roles directory if it doesn't exist
      if (!fs.existsSync(rolesDestPath)) {
        fs.mkdirSync(rolesDestPath, { recursive: true });
      }

      // Copy each role directory
      const roles = fs.readdirSync(rolesSourcePath);
      for (const role of roles) {
        const roleSourcePath = path.join(rolesSourcePath, role);
        const roleDestPath = path.join(rolesDestPath, role);
        
        if (fs.statSync(roleSourcePath).isDirectory()) {
