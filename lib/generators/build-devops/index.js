...
const fs = require('fs');
const { validateProjectConfig, handlePrompt, createGitHubRepos, initializeAndPushGitRepo, updateGitHubSecrets, getConfigField } = require('../../lib/utils');
  ...
  async _setupGitHubRepository() {
    try {
      ...
      const { createRepo } = await handlePrompt(this, [{
        type: 'confirm',
        name: 'createRepo',
        message: `Would you like to create the ${appName}-devops repository?`,
        default: true
      }]);
      ...
      if (createRepo) {
        ...
        if (!appName) {
          this.log('❌ Required field missing in config: app_name');
        ...
        try {
          ...
          try {
            ...
            this.log(`Changed to directory: ${projectPath}`);
            ...
            this.log(`Returned to directory: ${originalDir}`);
          }
        } catch (error) {
